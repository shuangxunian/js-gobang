...
export default class Evaluate {
  ...
  constructor(size = 15) {
    ...
    this.blackScores = Array.from({ length: size }).map(
      () => Array.from({ length: size }).fill(0) as number[],
    )
    this.whiteScores = Array.from({ length: size }).map(
      () => Array.from({ length: size }).fill(0) as number[],
    )
    this.initPoints()
  ...
  private initPoints(): void {
    ...
    for (const role of [1, -1]) {
      ...
      for (const direction of [0, 1, 2, 3]) {
        this.shapeCache[role][direction] = Array.from({ length: this.size }).map(
          () => Array.from({ length: this.size }).fill(shapes.NONE) as number[],
        )
      }
    }
    // 缓存每个形状对应的点位
  ...
  getPointsInLine(role: Role): PointsMap | false {
    const pointsInLine: PointsMap = {} // 在一条线上的点位
    let hasPointsInLine = false
    Object.keys(shapes).forEach((key) => {
      pointsInLine[shapes[key as keyof typeof shapes]] = new Set()
    })
    const last2Points = this.history.slice(-config.inlineCount).map(([position, role]) => position)
            ...
            if (vct) {
              if (depth % 2 === 0) {
                ...
                if (depth > 0) {
                  if (
                    shape === shapes.THREE &&
                    getAllShapesOfPoint(this.shapeCache, i, j, r as Role).length === 1
                  )
                    continue
                  if (
  ...
  getMoves(role: Role, depth: number, onThree = false, onlyFour = false): [number, number][] {
    const moves = Array.from(this._getMoves(role, depth, onThree, onlyFour)).map(
      (move) => [Math.floor(move / this.size), move % this.size] as [number, number],
    )
    return moves
  }
  ...
  private _getMoves(role: Role, depth: number, onlyThree = false, onlyFour = false): Set<number> {
    ...
    const res = new Set(
      [...blockfours, ...threes, ...blockthrees, ...two_twos, ...twos].slice(0, config.pointsLimit),
    )
    return res
  }
